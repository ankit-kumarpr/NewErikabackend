...
const createBill = async (req, res) => {
  ...
  try {
    ...
    // Convert bill_date to a Date object for comparison
    const billDateObj = new Date(bill_date);
    const startOfDay = new Date(billDateObj.setHours(0, 0, 0, 0));
    const endOfDay = new Date(billDateObj.setHours(23, 59, 59, 999));

    // Fetch quotations for the client, filtering by the selected bill date
    const quotations = await Quotation.find({
      client_id,
      createdAt: { $gte: startOfDay, $lte: endOfDay }, // Filter quotations by date
    });
    ...
    const productDetails = quotations.map((quotation) => {
      ...
      const totalPrice = product_price * purchase_quantity; // Ensure total price is calculated correctly
    ...
    const currentYearMonth = new Date(bill_date)
      .toISOString()
      .slice(0, 7)
      .replace("-", "");
    const lastBill = await Bill.findOne({
      invoice_no: new RegExp(`^erika${currentYearMonth}`),
    })
      .sort({ invoice_no: -1 })
      .exec();
    ...
    const newBill = new Bill({
      ...
      invoice_no,
      bill_date: new Date(bill_date), // Use the user-defined date
    });
...
module.exports = { createBill,Getallquotations,DeleteBill };
